<script lang="ts">
  ...
  onMount(() => {
    ...
    const interval = setInterval(async () => {
      ...
      await saveDraft({ id: p.id, title: p.title, widthPx: Math.round(p.widthIn * p.ppi), heightPx: Math.round(p.heightIn * p.ppi), canvasJSON: json, updatedAt: Date.now(), assetIds: [] });
    }, 5000);
  ...
  function bringForward() { canvas.bringForward(canvas.getActiveObject() as any); canvas.requestRenderAll(); }
  function sendBackwards() { canvas.sendBackwards(canvas.getActiveObject() as any); canvas.requestRenderAll(); }
  function deleteActive() { const o = canvas.getActiveObject(); if (o) { canvas.remove(o); canvas.requestRenderAll(); } }
...
<div class="editor" bind:this={hostEl}>
  <div class="toolbar">
    <input id="fileinput" type="file" accept="image/*" multiple on:change={onFilesChange} />
    <sp-action-group>
  ...
  <div class="status">
    ...
    <sp-button variant="primary" on:click={exportPng}>Export PNG (300 ppi)</sp-button>
  </div>
...
<style>
  ...
  .toolbar { display: flex; align-items: center; gap: 8px; padding: 8px; border-bottom: 1px solid #e5e5e5; }
  .toolbar .sep { width: 1px; height: 28px; background: #e5e5e5; }
  .stage { display: flex; align-items: center; justify-content: center; padding: 8px; overflow: auto; }
  canvas { background: #fff; box-shadow: 0 0 0 1px #e5e5e5; }
</style>
